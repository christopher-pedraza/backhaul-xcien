  ...
  const computeFlow = useCallback(async () => {
    ...
    try {
      ...
      for (const { from, to, capacity } of directedEdges) {
        ...
        lp.bounds!.push({ name: fName, type: glpk.GLP_LO, lb: 0, ub: Infinity });
        lp.bounds!.push({ name: oName, type: glpk.GLP_LO, lb: 0, ub: Infinity });
      ...
      // Extract flows
      const flows: FlowSolution['flows'] = directedEdges.map(({ from, to }) => {
        const fName = `f_${from}->${to}`;
        return {
          from,
          to,
